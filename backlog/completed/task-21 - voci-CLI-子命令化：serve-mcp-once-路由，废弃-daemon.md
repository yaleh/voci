...
Added dispatch() router in cmd/voci/main.go that maps bare subcommands to run():
- voci serve → --serve (Monitor-host, port 9474, stdout sink)
- voci mcp   → --session=integrated (MCP server, port 9473)
- voci once  → one-shot pipeline (--file etc.)
- leading-flag args fall through to legacy run() verbatim (backward compat)
- unknown bare tokens error listing serve/mcp/once

--daemon branch prints deprecation notice via injected stdout before proceeding.
SKILL.md command and contracts updated from voci --serve to voci serve.

Files: cmd/voci/main.go, cmd/voci/main_test.go, .claude/skills/voci-listen/SKILL.md
Commit: 2f8350c
