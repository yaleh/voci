...
### 交付内容
- **Phase A**: `internal/daemon/server.go` 新增 `//go:embed web/*` + `http.FileServerFS` 静态路由；`GET /` 返回 index.html，`GET /recorder.js` 返回 JS；API 路由不被截获（ServeMux 最长匹配）。
- **Phase B**: `emitRequest` 新增可选 `Kind string json:"kind"` 字段；`handleEmit` 透传 Kind，空时回退 `direct_prompt`。向后兼容旧 `{text}` 客户端。
- **Phase C**: `internal/daemon/web/index.html`（PTT 提示 + RAW/REWRITTEN 预览 + 可编辑 textarea + Confirm/Cancel 按钮）与 `internal/daemon/web/recorder.js`（Space PTT、MediaRecorder、两步提交）写入 embed.FS，随二进制分发，无构建步骤。
- **Tests**: `internal/daemon/static_test.go`（7 个测试：GET /、GET /recorder.js、API 不被 FileServer 截获、embed 内容断言）；`server_test.go` 新增 `TestEmit_PreservesKind`、`TestEmit_DefaultsKindWhenAbsent`。
...
## Definition of Done
<!-- DOD:BEGIN -->
- [x] #1 go test ./internal/daemon/...
- [x] #2 grep -q 'go:embed' internal/daemon/server.go
- [x] #3 grep -q 'FileServerFS' internal/daemon/server.go
- [x] #4 test -f internal/daemon/web/index.html
- [x] #5 test -f internal/daemon/web/recorder.js
- [x] #6 go test ./internal/daemon/...
- [x] #7 grep -q 'Kind' internal/daemon/server.go
- [x] #8 grep -q 'TestEmit_PreservesKind' internal/daemon/server_test.go
- [x] #9 go test ./internal/daemon/...
- [x] #10 grep -q 'Rewritten' internal/daemon/web/index.html
- [x] #11 grep -q '/api/voice/transcribe' internal/daemon/web/recorder.js
- [x] #12 grep -q '/api/voice/emit' internal/daemon/web/recorder.js
- [x] #13 grep -q 'MediaRecorder' internal/daemon/web/recorder.js
- [x] #14 go test ./...
- [x] #15 go build ./cmd/voci
- [x] #16 grep -q 'FileServerFS' internal/daemon/server.go
- [x] #17 grep -q 'go:embed' internal/daemon/server.go
- [x] #18 test -f internal/daemon/web/index.html
- [x] #19 test -f internal/daemon/web/recorder.js
- [x] #20 ! grep -q 'react\|vite\|package.json' internal/daemon/web/index.html
<!-- DOD:END -->
