"""Run SenseVoiceSmall against all 35 testcases. Execute from project root."""
...
# Load sensevoice adapter
_sv_spec = importlib.util.spec_from_file_location(
    "sensevoice_adapter", str(_this_dir / 'adapters' / 'sensevoice_adapter.py'))
_sv_mod = importlib.util.module_from_spec(_sv_spec)
_sv_spec.loader.exec_module(_sv_mod)
SenseVoiceSmallAdapter = _sv_mod.SenseVoiceSmallAdapter
...
def main():
    ...
    adapter = SenseVoiceSmallAdapter()
    results_dir = _this_dir / 'results'
    results_dir.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d-%H%M%S")
    out_path = results_dir / f"sensevoice-{ts}.jsonl"
